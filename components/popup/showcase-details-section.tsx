...
const ShowcaseDetailsSection = () => {
  ...
  return (
    <section className=" px-5 pb-10 pt-8 text-white sm:px-7 lg:px-8">
      <div className="mx-auto max-w-[1220px]">
        <div className="grid gap-10 border-b border-white/10 pb-10 lg:grid-cols-[220px_minmax(0,1fr)_280px]">
          <aside className="border-b border-white/10 pb-6 lg:border-b-0 lg:border-r lg:border-white/10 lg:pb-0 lg:pr-8">
            <h3 className="text-[0.75rem] font-bold uppercase tracking-[0.16em] text-white/42">
              Project Details
            </h3>
            ...
            <dl className="mt-5 space-y-4">
              {projectFacts.map(([label, value]) => (
                <div key={label}>
                  <dt className="text-[0.7rem] font-bold uppercase tracking-[0.14em] text-white/34">
                    {label}
                  </dt>
                  <dd className="mt-1 text-sm font-semibold leading-[1.5] text-white">
                    {value}
                  </dd>
                </div>
              ))}
          ...
          <div className="min-w-0">
            <h3 className="text-xl font-bold leading-tight tracking-[-0.03em] text-white sm:text-2xl">
              DesignA11Y gives accessibility teams a sharper system to explain the workflow and build trust faster.
            </h3>

            <div className="mt-5 space-y-4 text-[0.95rem] font-medium leading-[1.75] text-white/68 sm:text-base">
              <p>
          ...
          <div className="space-y-4 lg:pl-2">
            {supportNotes.map((note) => (
              <div
                key={note}
                className="rounded-[18px] border border-white/10 bg-[#11110d] px-5 py-5 shadow-[0_10px_34px_rgba(0,0,0,0.28)]"
              >
                <p className="text-sm font-medium leading-[1.7] text-white/66">
                  {note}
                </p>
              </div>
            ))}
        ...
        <div className="grid gap-8 pt-8 lg:grid-cols-[minmax(0,1fr)_280px]">
              ...
              return (
                <div key={item.title} className="border-b border-white/10">
                  <button
                    ...
                    className="flex w-full items-center justify-between py-4 text-left transition hover:text-[#b8ea18]"
                  >
                  ...
                  {isOpen ? (
                    <div className="pb-4 pr-8 text-sm font-medium leading-[1.75] text-white/66">
                      {item.body}
                    </div>
                  ) : null}
                </div>
          ...
          <div className="rounded-[20px] border border-white/10 bg-[#11110d] px-5 py-6 shadow-[0_14px_40px_rgba(0,0,0,0.24)]">
            <h4 className="text-lg font-bold tracking-[-0.03em] text-white">
              Final Outcome With 100XLIFT
            </h4>
            <p className="mt-4 text-sm font-medium leading-[1.75] text-white/66">
